    ...
    def post(self, request, *args, **kwargs):
        ...
        if client_id:
            try:
                ...
                return Response({"message": "L'ID du client spécifié n'existe pas."}, status=status.HTTP_400_BAD_REQUEST)

        contenu = data.get('contenu')

        if not contenu:
            return Response({"message": "Veuillez fournir un contenu de commentaire."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        comment = commentaire.create_comment(contenu=contenu, avocat=avocat_obj, client=client_obj)
        serializer = CommentaireSerializer(comment)
        return Response(serializer.data, status=status.HTTP_201_CREATED)
